...
def init(): # 초기 설정
    width = 120; height = 150; rate = 0.95; input_img_num = 310; output_img_num = 93
    return width, height, rate, input_img_num, output_img_num
...
def test(): # 자기가 원하는 이미지 넣어서 테스트 하는 단계
   for i in range(output_img_num):
       ...
       cv2.imshow("input_img", input_img) # 입력영상 출력
       cv2.moveWindow('input_img', 700, 300)
       ...
       img = cv2.resize(img, (width, height))
       cv2.imshow("img", img)           # 출력영상
       cv2.moveWindow('img', 500, 300)
       n = cv2.waitKey(1000)
       if n == 27: break                # esc 누르면 프로그램 바로 종료
       elif n == ord('s'):              # s를 누를 경우 지금 보고 있는 이미지 정지
           cv2.waitKey(100000)

width, height, rate, input_img_num, output_img_num = init()
